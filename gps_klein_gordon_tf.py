...
def initial_cond_2(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[1], 0.0)


def initial_value_2(x, y, _):
...
class GeneralEnv(gym.Env):
    def __init__(self, env_config: Optional[Dict] = None):
        ...
        self._b = env_config['data_config']['b']
        self._g = env_config['data_config']['g']
        self._k = env_config['data_config']['k']
    ...
    def _boundaries(geom: dde.geometry.Geometry) -> List[dde.icbc.BC]:
        ...
        ic2 = dde.icbc.OperatorBC(geom, initial_value_2, initial_cond_2)
        return [bc_left, bc_right, ic, ic2]

    def _construct_data(self, geom: dde.geometry.Geometry, icbc_list: List[dde.icbc.BC]) -> dde.data.Data:
        return dde.data.PDE(geom, self._pde, icbc_list, num_domain=self._num_domain, num_boundary=self._num_boundary)
...
def main():
    ...
    ic = dde.icbc.DirichletBC(geom, initial_value, initial_cond)
    ic2 = dde.icbc.OperatorBC(geom, initial_value_2, initial_cond_2)
    data = dde.data.PDE(geom, pde, [bc_left, bc_right, ic, ic2], num_domain=num_domain, num_boundary=num_boundaries)
